...
public class Tencent02 {
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        if ((totalLength & 1) == 1) {
            ...
            double median = getKthElemetn(nums1, nums2, mid + 1) / 1.0;
            return median;
        } else {
            int mid1 = (totalLength - 1) >> 1;
            int mid2 = mid1 + 1;
            double median = (getKthElemetn(nums1, nums2, mid1 + 1) + getKthElemetn(nums1, nums2, mid2 + 1)) / 2.0;
            return median;
        }
    }

    private static int getKthElemetn(int[] nums1, int[] nums2, int k) {
        int idx1 = 0;
        int idx2 = 0;
        while (true) {
